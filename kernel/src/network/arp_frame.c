 ...
 * into I:I:I:I:I:I
 */
void pack_mac(uint8_t* dest, char* src) {
	for (int i = 0, j = 0; i < 17; i += 3) {
		uint32_t i1 = hex_to_int(src[i]);
		uint32_t i2 = hex_to_int(src[i+1]);
		dest[j++] = (i1<<4) + i2;
	}
}

uint32_t get_ip (const char* ip, uint32_t len) {
    uint32_t ipv4  = 0;
    char arr[4];
    int n1 = 0;
     ...
     for (int i =0; i<len; i++) {
        ...
        if (ch == '.') {
            ...
            n1 = 0;
            ip_vals[n2++] = atoi(arr);
       	    //printf("Check ipval:%d , arr:%s",ip_vals[n2],arr);
	} else {
        ...
        n1 = 0;
        ip_vals[n2++] = atoi(arr);
        //printf("Final Check ipval:%d , arr:%s",ip_vals[n2],arr);
...
int parse_arp_packet(const struct ethr_hdr *eth, const char *my_ip, bool expected_reply, uint8_t *received_from_mac, char *received_from_ip, uint8_t *mac_msg) {
	...
	if (strcmp((const char*)my_mac, (const char*)dst_mac)) {
		printf("Not the intended recipient\n");
		return -1;
	}
        */
	...
	if (strcmp(my_ip, (const char*)dst_ip)) {
	    printf("Not the intended recipient! Expected %s, got %s\n", my_ip, dst_ip);
	    return -1;
	}
