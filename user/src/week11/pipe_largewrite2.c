...
int main() {
    printf("pipe large write test 2 begins.\n");
    int fd[2];
    ...
    if (pid < 0) {
        ...
        // 从管道读取数据
        nbytes = read(fd[0], read_msg, sizeof(read_msg)); //128
        if (nbytes <= 0) {
        ...
        if(total_read != PIPE_SIZE){
            printf("pipe large write 2 failed: expect to read %d bytes but get %d instead.\n", PIPE_SIZE, total_read);
            return 1;
        }
        ...
        int status;
        wait(&status);
        if(!status) printf("pipe large write test 2 passed.\n");
    }
